#!/usr/bin/env python3
"""
Reads 'dora' from ports.json, calls /api/v1/slots,
and saves the response to data/dora_slots.json.
"""
...
OUT_PATH = os.path.join("data", "dora_slots_list.json")
...
def main():
    ...
    base = ports.get("dora")
    if not base:
        print("Key 'dora' not found in ports.json", file=sys.stderr)
        sys.exit(1)

    url = base.rstrip("/") + "/api/v1/slots"
    ...
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "fetch_dora_slots/0.1",
        },
    )
    try:

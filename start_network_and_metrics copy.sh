...
python monitoring/python/live_collection/tx_metrics_ws.py --duration 900 &
monitor_pids+=($!)
...
python monitoring/python/live_collection/cpu_mem_net_colletion.py --duration 900 --interval 1 &
monitor_pids+=($!)

set +x
echo
echo ">>> Monitoring scripts started in background (PIDs: ${monitor_pids[*]})"
echo ">>> They will run for up to 900 seconds."
set -x
